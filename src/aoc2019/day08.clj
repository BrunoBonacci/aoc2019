...
(def image (read-image (slurp (io/resource "day08-image-input.txt")) [25 6]))
...
(fact
 "solution: what is the number of 1 digits multiplied by the number of 2 digits?"
...
;; top-right pixel is white because the top layer is 2 (transparent),
;; but the second layer is 1.  The bottom-left pixel is white because
;; the top two layers are 2, but the third layer is 1.  The
;; bottom-right pixel is black because the only visible pixel in that
...
(fact
 "decode test image"
 (decode (read-image  "0222112222120000" [2 2]) [2 2]) => "01\n10"
 )
