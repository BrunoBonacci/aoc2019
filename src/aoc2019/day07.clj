...
;;     thrusters. However, this phase setting sequence may not have
;;     been the best one; another sequence might have sent a higher
...
(defn amplifier
  [prog phase]
  (fn [signal]
    (-> (machine-eval prog [phase signal]) :output first)))
...
(fact
 "testing amplifiers circuit 1"

 ((circuit (parse "3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0")
           [4 3 2 1 0])
  0) => 43210
...
(fact
 "testing amplifiers circuit 2"

 ((circuit (parse "3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0")
           [0 1 2 3 4])
  0) => 54321
...
(fact
 "testing amplifiers circuit 3"

 ((circuit (parse "3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0")
           [1 0 4 3 2])
  0) => 65210
...
(def amplifier-program (parse (slurp (io/resource "day07-amplifier-program.txt"))))


(defn best-phase-settings
  [prog]
  (->>
   (combo/permutations [4 3 2 1 0])
   (pmap (juxt identity #((circuit prog %) 0)))
   (sort-by second >)
   first))


(facts
 "test best phase settings"
...
(facts
 "solution: What is the highest signal that can be sent to the thrusters?"

 (second
  (best-phase-settings amplifier-program))
 => 17440
 )
