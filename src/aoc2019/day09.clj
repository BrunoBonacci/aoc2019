...
;; you receive a faint distress signal coming from the asteroid
...
;;
;; While BOOST (your puzzle input) is capable of boosting your
;; sensors, for tenuous safety reasons, it refuses to do so until the
;; computer it runs on passes some checks to demonstrate it is a
...
;; parameters and position mode parameters with the same value refer
;; to the same address.
;;
...
;; is negative) by the value of the parameter.  For example, if the
;; relative base is 2000, then after the instruction 109,19, the
;; relative base would be 2019. If the next instruction were 204,-34,
...
;;     instructions near the beginning of the BOOST program will
...
(fact
 "quine program example"

 (let [p (parse "109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99")]
   (->> (machine-eval p [])
      :output
      ) => p)
 )
...
(fact
 ...
 (let [p (parse "1102,34915192,34915192,7,4,7,99,0")]
   (->> (machine-eval p [])
      :output
      first))
 => 1219070632396864
...
(fact
 ...
 (let [p (parse "104,1125899906842624,99")]
   (->> (machine-eval p [])
      :output
      first))
 => 1125899906842624
...
(fact
 "solution: What BOOST keycode does it produce?"

 (let [p (parse (slurp (io/resource "day09-boost-program.txt")))]
   (->> (machine-eval p [1])
      :output
      first))
...
(fact
 "solution: What are the coordinates of the distress signal?"
