...
;;
;; You've landed at the Universal Orbit Map facility on
;; Mercury. Because navigation in space often involves transferring
;; between orbits, the orbit maps here are useful for finding
;; efficient routes between, for example, you and Santa. You download
;; a map of the local orbits (your puzzle input).
;;
;; Except for the universal Center of Mass (COM), every object in
;; space is in orbit around exactly one other object. An orbit looks
;; roughly like this:
;;
;; ```
;;                   \
...
;; that BBB takes around AAA (drawn with lines) is only partly
;; shown. In the map data, this orbital relationship is written
...
(defn checksum
  [omap]
  (->>
   (la/pre-traverse omap "COM")
   (reduce (fn [m n]
        (let [sux   (->> (lg/successors omap n) (remove (set (keys m))))
              score (inc (get m n))]
          (into m (map (fn [i] [i score]) sux))))
      {"COM" 0})
   (map second)
   (reduce +)))
...
(fact
 "verify orbit map checksum"

 (checksum (orbit-map test-omap)) => 42
 )



(def omap (slurp (io/resource "day06-orbit-map.txt")))


(fact
 "What is the total number of direct and indirect orbits in your map data?"

 (checksum (orbit-map omap)) => 158090
 )
...
(defn minimum-orbital-transfers
  [omap obj1 obj2]
  (-> omap
     (la/shortest-path obj1 obj2)
     count
     (- 3)))
...
(fact
 "minimum orbital transfer test"

 (minimum-orbital-transfers (orbit-map test-omap2) "YOU" "SAN") => 4
 )



(fact
 "solution: What is the minimum number of orbital transfers required?"

 (minimum-orbital-transfers (orbit-map omap) "YOU" "SAN") => 241
 )
