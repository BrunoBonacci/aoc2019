...
;; gravity. Then, once all moons' velocities have been updated, update
;; the position of every moon by applying velocity. Time progresses by
...
;; ```
;;
;; Then, it might help to calculate the total energy in the
;; system. The total energy for a single moon is its potential energy
;; multiplied by its kinetic energy. A moon's potential energy is the
;; sum of the absolute values of its x, y, and z position
...
;; moons after 10 steps produces the total energy in the system, 179.
;;
...
(defn parse
  ...
  (->> input
     (str/split-lines)
     (map (partial re-find #"<x=([0-9-]+), y=([0-9-]+), z=([0-9-]+)>"))
     (map rest)
     (map (partial map read-string))
     (map (fn [[x y z]] {:x x :y y :z z, :vx 0 :vy 0 :vz 0}))))
...
(defn attract
  ...
  (->> (for [b moons :when (not= m b)]
       {:x (sign (- (get b :x) (get m :x)))
        :y (sign (- (get b :y) (get m :y)))
        :z (sign (- (get b :z) (get m :z)))})
     (apply merge-with +)
     ((fn [{:keys [x y z]}]
        (as-> m $
          (update $ :vx + x)
...
(defn step
  [moons]
  (map (partial attract moons) moons))


(fact
 "calculating next position"
 (->
  (iterate step (parse test-input))
  (nth 10))
...
(defn energy
  [{:keys [x y z vx vy vz]}]
  (let [pot (+ (Math/abs x)  (Math/abs y)  (Math/abs z))
        kin (+ (Math/abs vx) (Math/abs vy) (Math/abs vz))]
    {:pot pot :kin kin :tot (* pot kin)}))


(defn total-energy-at
  [moons n]
  (->
   (iterate step moons)
   (nth n)
...
(fact
 "calculating 10th position total energy"

 (total-energy-at (parse test-input) 10)
 => 179
...
(fact
 "calculating 100th position total energy"

 (total-energy-at (parse test-input2) 100)
 => 1940
...
(fact
 "solution: calculating 1000th position total energy"

 (total-energy-at (parse input) 1000)
 => 14809
