...
;;
;; You're starting to sweat as the ship makes its way toward
;; Mercury. The Elves suggest that you get the air conditioner working
;; by upgrading your ship computer to support the Thermal Environment
;; Supervision Terminal.
;;
;; The Thermal Environment Supervision Terminal (TEST) starts by
;; running a diagnostic program (your puzzle input). The TEST
;; diagnostic program will run on your existing Intcode computer after
;; a few modifications:
;;
...
;; that explains what should be connected to the input and output. The
...
;; mode 0, position mode, which causes the parameter to be interpreted
;; as a position - if the parameter is 50, its value is the value
;; stored at address 50 in memory. Until now, all parameters have been
;; in position mode.
;;
;; Now, your ship computer will also need to handle parameters in mode
;; 1, immediate mode. In immediate mode, a parameter is interpreted as
;; a value - if the parameter is 50, its value is simply 50.
;;
...
;; single digits, one per parameter, read right-to-left from the
;; opcode: the first parameter's mode is in the hundreds digit, the
;; second parameter's mode is in the thousands digit, the third
;; parameter's mode is in the ten-thousands digit, and so on. Any
;; missing modes are 0.
;;
...
;; 0 (hundreds digit), 1 (thousands digit), and 0 (ten-thousands
...
;; position mode, which also works like it did before - 99 is written
;; to address 4.
;;
...
;; is no longer always 4.  Integers can be negative: 1101,100,-1,4,0
...
;; various parts of the Intcode computer, like parameter modes,
;; function correctly. For each test, it will run an output
;; instruction indicating how far the result of the test was from the
;; expected value, where 0 means the test was successful. Non-zero
;; outputs mean that a function is not working correctly; check the
;; instructions that were run before the output instruction to see
;; which one failed.
;;
;; Finally, the program will output a diagnostic code and immediately
;; halt. This final output isn't an error; an output followed
;; immediately by a halt means the program finished. If all outputs
;; were zero except the diagnostic code, the diagnostic program ran
;; successfully.
;;
...
(fact
 "test new instructions and negative values"

 (:state (machine-eval (parse "1101,100,-1,4,0")))
 => [1101 100 -1 4 99]
 )
...
(fact
 "solving conditioner problem"

  (->> (machine-eval (parse program) [1])
     :output
     last)
   => 10987514
  ;;
...
;;     non-zero, it sets the instruction pointer to the value from the
