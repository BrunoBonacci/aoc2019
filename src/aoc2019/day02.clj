...
;; escaped. "That computer ran Intcode programs like the gravity
...
;; 1,0,0,3,99). To run one, start by looking at the first integer
;; (called position 0). Here, you will find an opcode - either 1, 2,
;; or 99. The opcode indicates what to do; for example, 99 means that
;; the program is finished and should immediately halt. Encountering
...
;; works just like the previous, but it multiplies instead of
;; adding. The inputs are at positions 3 and 11; these positions
;; contain 70 and 50 respectively. Multiplying these produces 3500;
...
;; alarm" state it had just before the last computer caught fire. To
;; do this, before running the program, replace position 1 with the
;; value 12 and replace position 2 with the value 2. What value is
;; left at position 0 after the program halts?
;;


(defmulti eval-instruction (fn [[state pp]] (get state pp)))
...
(defmethod eval-instruction 1
  [[state pp :as arg]]
  (let [[op a b c] (instruction arg 4)]
    [(->>
      (+' (get state a) (get state b))
      (assoc state c))
     (+ 4 pp)]))
...
;; --- Part Two ---
;;
;; "Good, the new computer seems to be working correctly! Keep it
;; nearby during this mission - you'll probably use it again. Real
;; Intcode computers support many more features than your new one, but
;; we'll let you know what they are as you need them."
;;
;; "However, your current priority should be to complete your gravity
;; assist around the Moon. For this mission to succeed, we should
...
;; used as the initial state for the computer's memory. When you run
;; an Intcode program, make sure to start by initializing memory to
;; the program's values. A position in memory is called an address (for
;; example, the first value in memory is at "address 0").
;;
;; Opcodes (like 1, 2, or 99) mark the beginning of an
;; instruction. The values used immediately after an opcode, if any,
;; are called the instruction's parameters. For example, in the
;; instruction 1,2,3,4, 1 is the opcode; 2, 3, and 4 are the
...
;; instruction; until you add more instructions to the computer, this
;; is always 4 (1 opcode + 3 parameters) for the add and multiply
;; instructions. (The halt instruction would increase the instruction
;; pointer by 1, but it halts the program instead.
;;
;; "With terminology out of the way, we're ready to proceed. To
;; complete the gravity assist, you need to determine what pair of
