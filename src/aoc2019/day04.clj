(ns aoc2019.day04
  (:require [midje.sweet :refer :all]
            [clojure.string :as str]
            [clojure.set :as set]))


;; --- Day 4: Secure Container ---
;;
;; You arrive at the Venus fuel depot only to discover it's protected
;; by a password. The Elves had written the password on a sticky note,
;; but someone threw it out.
;;
;; However, they do remember a few key facts about the password:
;;
;; It is a six-digit number.
;;   - The value is within the range given in your puzzle input.
;;   - Two adjacent digits are the same (like 22 in 122345).
;;   - Going from left to right, the digits never decrease; they only
;;     ever increase or stay the same (like 111123 or 135679).
;;
;; Other than the range rule, the following are true:
;;
;; `111111` meets these criteria (double 11, never decreases).
;; `223450` does not meet these criteria (decreasing pair of digits 50).
;; `123789`does not meet these criteria (no double).
;;
;; How many different passwords within the range given in your puzzle
;; input meet these criteria?
;;
;; Your puzzle input is `145852-616942`.

(defn increasing
  [n]
  (apply <= (map int (str n))))

(fact
 "increasing order"

 (increasing "123456") => true
 (increasing "111111") => true
 (increasing "123334") => true
 (increasing "123324") => false
 (increasing "223450") => false
 )


(defn double-digit
  [n]
  (->> (str n)
     (partition 2 1)
     (some (fn [[a b]] (= a b)))))


(fact
 "two consecutive digits must be the same"

 (double-digit 123456) => falsey
 (double-digit 121212) => falsey
 (double-digit 113456) => truthy
 (double-digit 111356) => truthy
 (double-digit 122356) => truthy
 )


(->> (range 145852 (inc 616942))
   (filter #(and (increasing %) (double-digit %)))
   (count))
;;=> 1767



;; --- Part Two ---
;;
;; An Elf just remembered one more important detail: the two adjacent
;; matching digits are not part of a larger group of matching digits.
;;
;; Given this additional criterion, but still ignoring the range rule,
;; the following are now true:
;;
;; `112233` meets these criteria because the digits never decrease and all repeated digits are exactly two digits long.
;; `123444` no longer meets the criteria (the repeated 44 is part of a larger group of 444).
;; `111122` meets the criteria (even though 1 is repeated more than twice, it still contains a double 22).
;;
;; How many different passwords within the range given in your puzzle
;; input meet all of the criteria?


(defn digit-groups
  [n]
  (->> (str n)
     (partition-by identity)
     (filter #(>= (count %) 2))))


(defn no-larger-group
  [n]
  (= 2 (reduce min (map count (digit-groups n)))))


(def valid-pass #(and (increasing %) (double-digit %) (no-larger-group %)))

(facts
 "valid password rules for doubles"

 (valid-pass "112233") => true
 (valid-pass "123444") => false
 (valid-pass "111122") => true
 (valid-pass "111222") => false
 )


(->> (range 145852 (inc 616942))
   (filter valid-pass)
   (count))
;;=> 1192
